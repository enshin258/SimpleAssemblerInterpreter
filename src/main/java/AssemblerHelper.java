...
class AssemblerHelper {
    ...
    void addRegisterValueToList(GrammarParser.Source_registerContext ctx)
    {
    ...
    void setTargetRegister(GrammarParser.Target_registerContext ctx) {
        String registerTextRepresentation = prepareStringFromContextText(ctx.getText());
        switch (registerTextRepresentation)
        {
            case "eax":
    ...
    void prepareDataStringBasedOnInput()
    {
        ...
        for (Object o:this.mathOperationElementsList) {
            ...
            {
                if(o==this.mathOperationElementsList.get(this.mathOperationElementsList.size()-1))
                {
                    stringResult.append(o.toString());
                }
                else
                {
